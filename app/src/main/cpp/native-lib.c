...
//音频播放
JNIEXPORT void JNICALL
Java_com_georgeren_myndk2_VideoPlayer_play(JNIEnv *env, jobject instance, jstring input_) {
    const char *input = (*env)->GetStringUTFChars(env, input_, 0);
    ...
    //AudioTrack对象,com.georgeren.myndk2.VideoPlayer中的方法createAudioTrack
...
//视频解码
JNIEXPORT void JNICALL
Java_com_georgeren_myndk2_VideoUtils_decode(JNIEnv *env, jclass type, jstring input_,
                                            jstring outptu_) {
    LOGE("%s", "decode");
    //需要转码的视频文件(输入的视频文件)
    const char *input = (*env)->GetStringUTFChars(env, input_, 0);
...
//音频解码（重采样）
JNIEXPORT void JNICALL
Java_com_georgeren_myndk2_VideoUtils_sound(JNIEnv *env, jclass type, jstring input_,
                                           jstring output_) {
    const char *input = (*env)->GetStringUTFChars(env, input_, 0);
